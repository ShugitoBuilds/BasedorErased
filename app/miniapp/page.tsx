...
function MiniAppContent() {
  /* DYNAMIC MARKET ID LOGIC */
  const searchParams = useSearchParams();
  const marketIdParam = searchParams.get('marketId');
  const marketId = marketIdParam ? parseInt(marketIdParam) : 1; // Default to 1 if missing or invalid
  ...
  useEffect(() => {
    if (isConfirmed) {
      ...
      // Delayed refresh to handle indexer lag (1s and 4s)
  ...
  useEffect(() => {
    ...
    initMiniApp();
  }, [marketId]); // Re-run if marketId changes
  ...
  const handleBet = async (isMoon: boolean) => {
    ...
    try {
      ...
      // Check USDC balance
      if (usdcBalance !== undefined && usdcBalance < betAmount) {
        setError(
          `Insufficient USDC balance on ${REQUIRED_CHAIN_NAME}. \n` +
          `You have ${formatUSDC(usdcBalance)} USDC. You need at least 1 USDC to bet.\n` +
      ...
      // Check allowance
      if (!allowance || allowance < betAmount) {
      ...
      await writeContractAsync({
        ...
        args: [BigInt(marketId), betAmount],
      });
    } catch (err) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-md p-4 flex flex-col gap-4">
        ...
        {/* Transaction Status - Fixed Top/Overlay or just top of flow */}
        {isConfirming && (
        ...
        {/* Market Card with Betting Buttons Integrated */}
